 ...
 * Gallery cache-bust is batch-multi-w3-v1 (wave 1/2 stay on their own busts).
 */
...
def write_cache_bust() -> None:
    dest = ROOT / "tmp/cache-bust-shop-batch-multi-w3-v1"
...
def main() -> None:
    ...
    if CACHE_BUST != "batch-multi-w3-v1":
        raise SystemExit("wave 3 cache bust must be batch-multi-w3-v1")
    write_web()
    write_api()
    write_price_index()
    write_cache_bust()

...
for (const [id, slug, price] of batchMulti) {
  ...
  const shots = productGallery(item);
  assert.equal(shots.length, 3, `${id} gallery has 3 unique angles`);
  assert.ok(shots[0].includes(`${slug}.jpg?v=batch-multi-w1-v4`), `${id} front is batch-multi-w1-v4`);
  assert.ok(shots[1].includes(`${slug}-2.jpg?v=batch-multi-w1-v4`), `${id} angle 2 is batch-multi-w1-v4`);
  assert.ok(shots[2].includes(`${slug}-3.jpg?v=batch-multi-w1-v4`), `${id} angle 3 is batch-multi-w1-v4`);
  assert.doesNotMatch(shots.join(' '), /purple|5c4d91|بنفش/i, `${id} gallery has no purple asset`);
}
...
for (const [id, slug, price] of batchMultiW2) {
  ...
  const shots = productGallery(item);
  assert.equal(shots.length, 3, `${id} gallery has 3 unique angles`);
  assert.ok(shots[0].includes(`${slug}.jpg?v=batch-multi-w2-v1`), `${id} front is batch-multi-w2-v1`);
  assert.ok(shots[1].includes(`${slug}-2.jpg?v=batch-multi-w2-v1`), `${id} angle 2 is batch-multi-w2-v1`);
  assert.ok(shots[2].includes(`${slug}-3.jpg?v=batch-multi-w2-v1`), `${id} angle 3 is batch-multi-w2-v1`);
  assert.doesNotMatch(shots.join(' '), /purple|5c4d91|بنفش/i, `${id} gallery has no purple asset`);
}
...
for (const [id, slug, price] of batchMultiW3) {
  ...
  const shots = productGallery(item);
  assert.equal(shots.length, 3, `${id} gallery has 3 unique angles`);
  assert.ok(shots[0].includes(`${slug}.jpg?v=batch-multi-w3-v1`), `${id} front is batch-multi-w3-v1`);
  assert.ok(shots[1].includes(`${slug}-2.jpg?v=batch-multi-w3-v1`), `${id} angle 2 is batch-multi-w3-v1`);
  assert.ok(shots[2].includes(`${slug}-3.jpg?v=batch-multi-w3-v1`), `${id} angle 3 is batch-multi-w3-v1`);
  assert.doesNotMatch(shots.join(' '), /purple|5c4d91|بنفش/i, `${id} gallery has no purple asset`);
}
...
for (const [id, slug, price] of batchMultiW4) {
  ...
  const shots = productGallery(item);
  assert.equal(shots.length, 3, `${id} gallery has 3 unique angles`);
  assert.ok(shots[0].includes(`${slug}.jpg?v=batch-multi-w4-v1`), `${id} front is batch-multi-w4-v1`);
  assert.ok(shots[1].includes(`${slug}-2.jpg?v=batch-multi-w4-v1`), `${id} angle 2 is batch-multi-w4-v1`);
  assert.ok(shots[2].includes(`${slug}-3.jpg?v=batch-multi-w4-v1`), `${id} angle 3 is batch-multi-w4-v1`);
  assert.doesNotMatch(shots.join(' '), /purple|5c4d91|بنفش/i, `${id} gallery has no purple asset`);
}

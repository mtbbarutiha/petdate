...
for (const [id, slug, price] of batchMultiW4) {
  ...
  const shots = productGallery(item);
  assert.equal(shots.length, 3, `${id} gallery has 3 unique angles`);
  assert.ok(shots[0].includes(`${slug}.jpg?v=batch-multi-w4-v4`), `${id} front is batch-multi-w4-v4`);
  assert.ok(shots[1].includes(`${slug}-2.jpg?v=batch-multi-w4-v4`), `${id} angle 2 is batch-multi-w4-v4`);
  assert.ok(shots[2].includes(`${slug}-3.jpg?v=batch-multi-w4-v4`), `${id} angle 3 is batch-multi-w4-v4`);
  assert.doesNotMatch(shots.join(' '), /purple|5c4d91|بنفش/i, `${id} gallery has no purple asset`);
}
...
const digikalaB1P4 = [
  ['p330', 'dog-food-dkp-20949593', 195_000],
  ['p338', 'rodent-supplies-dkp-2909989', 550_000],
  ['p341', 'rodent-supplies-dkp-11804534', 181_000],
] as const;
for (const [id, slug, price] of digikalaB1P4) {
  const item = getProduct(slug) ?? getProduct(id);
  assert.ok(item, `${id} exists`);
  assert.equal(item.priceToman, price, `${id} MANIFEST price`);
  const shots = productGallery(item);
  assert.equal(shots.length, 1, `${id} gallery is front-only (1 real angle)`);
  assert.ok(shots[0].includes(`${slug}.jpg?v=digikala-b1-p4-v1`), `${id} front is digikala-b1-p4-v1`);
  assert.ok(!shots[0].includes('-2.jpg') && !shots[0].includes('-3.jpg'), `${id} no fake angles`);
  assert.doesNotMatch(shots.join(' '), /purple|5c4d91|بنفش/i, `${id} gallery has no purple asset`);
}
assert.ok(getProduct('rodent-supplies-dkp-2909989'), 'Digikala Part4 Topfeed rodent is in catalog');

...
adminRouter.get('/games', (req, res) => {
  ...
  const games = dbService.listGames({
    ...
    gameType,
    province:
      typeof req.query.province === 'string' ? req.query.province.trim() || undefined : undefined,
    host:
      typeof req.query.host === 'string'
        ? req.query.host.trim() || undefined
        : typeof req.query.organizer === 'string'
          ? req.query.organizer.trim() || undefined
          : undefined,
  });
...
adminRouter.put('/shop/products/:id', (req, res) => {
  ...
  const product = adminPlatform.upsertShopProduct({
    ...
    images: Array.isArray(body.images) ? body.images.map(String) : existing.images,
...
adminRouter.post('/shop/catalog/sync', (req, res) => {
  ...
  const result = adminPlatform.replaceShopCatalog({
    products: products.map((p: Record<string, unknown>) => ({
      ...
      images: Array.isArray(p.images) ? (p.images as unknown[]).map(String) : undefined,

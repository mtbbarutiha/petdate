...
async function main() {
  ...
  assert(tip.includes('فرانک احمدی'), 'offline trainer introduces as Pasha');
  ...
  assert(puppyCanon.includes('فرانک احمدی') || /توله|جامعه|جلسه/.test(puppyCanon), 'puppy canon topic');
  ...
  const vetPhoto = offlineAiAdvice({
    kind: 'vet',
    agentName: 'دکتر سارا نوری',
    clinicalImage: { mimeType: 'image/jpeg', dataUrl: 'data:image/jpeg;base64,QQ==' },
    userMessage: 'این زخم پاشه',
  });
  assert(vetPhoto.includes('دکتر سارا نوری'), 'photo triage uses Sara persona name');
  ...
  assert(aiUser.name === 'فرانک احمدی', 'ai user profile name is Pasha');

  const session = await startAiFallbackConsult({ patient, serviceKind: 'trainer' });
  assert(session, 'ai session started');
  assert(decorateAiConsultDisplay(session!.consult).vetName === 'فرانک احمدی', 'display name');
  const msgs = dbService.listVetConsultChatMessages(session!.consult.id);
  assert(msgs[0]!.text.includes('فرانک احمدی'), 'intro mentions Pasha');

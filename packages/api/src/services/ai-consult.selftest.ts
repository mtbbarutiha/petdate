...
async function main() {
  ...
  assert(tip.includes('لیلا کیانی'), 'offline trainer introduces as Pasha');
  ...
  assert(puppyCanon.includes('لیلا کیانی') || /توله|جامعه|جلسه/.test(puppyCanon), 'puppy canon topic');
  ...
  assert(vetTip.includes('لیلا کیانی'), 'offline vet introduces as Pasha');
  ...
  assert(aiUser.name === 'لیلا کیانی', 'ai user profile name is Pasha');

  const session = await startAiFallbackConsult({ patient, serviceKind: 'trainer' });
  assert(session, 'ai session started');
  assert(decorateAiConsultDisplay(session!.consult).vetName === 'لیلا کیانی', 'display name');
  const msgs = dbService.listVetConsultChatMessages(session!.consult.id);
  assert(msgs[0]!.text.includes('لیلا کیانی'), 'intro mentions Pasha');
  ...
  // Vet AI fallback also displays as لیلا کیانی (not legacy «دستیار هوشمند»).
  ...
  assert(
    decorateAiConsultDisplay(vetSession!.consult).vetName === 'لیلا کیانی',
    'vet AI display name is Pasha'
  );
  const vetMsgs = dbService.listVetConsultChatMessages(vetSession!.consult.id);
  assert(vetMsgs[0]!.text.includes('لیلا کیانی'), 'vet opening mentions Pasha');

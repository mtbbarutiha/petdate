...
export function getCrmDashboard(actor: AdminAuthActor): CrmDashboard {
    ...
    const key =
      i.channel === 'call_in'
        ...
        : i.channel === 'call_out'
          ...
          : i.channel === 'whatsapp'
            ? 'واتساپ'
            : CRM_CHANNEL_LABELS[i.channel] || i.channel;
  ...
  const inProgress = openTickets.filter((t) =>
    ['در حال بررسی', 'تخصیص‌یافته', 'در انتظار مشتری', 'بازگشایی‌شده', 'جدید'].includes(t.status)
  ).length;
  const stalled = openTickets.filter((t) => t.status === 'در انتظار مشتری').length;
  const ticketStatus = [
    { key: 'in_progress', label: 'در جریان', value: Math.max(0, inProgress - stalled), color: '#0ba5f2' },
    { key: 'unassigned', label: 'تخصیص‌نیافته', value: unassigned, color: '#5c4d91' },
    { key: 'breached', label: 'نقض SLA', value: breachedSla, color: '#c62828' },
    { key: 'stalled', label: 'متوقف‌شده', value: stalled, color: '#15cca0' },
  ];

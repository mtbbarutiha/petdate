  ...
  if (!db) {
    ...
    const bootShopPilot = () => {
      ...
      try {
        ...
        console.warn('Shop batch-multi wave 1 seed skipped/failed:', (err as Error).message);
      }
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { seedShopBatchMultiWave2Products } =
          require('./data/shop-batch-multi-wave2-products') as typeof import('./data/shop-batch-multi-wave2-products');
        seedShopBatchMultiWave2Products();
      } catch (err) {
        console.warn('Shop batch-multi wave 2 seed skipped/failed:', (err as Error).message);
      }
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { seedShopBatchMultiWave3Products } =
          require('./data/shop-batch-multi-wave3-products') as typeof import('./data/shop-batch-multi-wave3-products');
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { seedShopBatchMultiWave5Products } =
          require('./data/shop-batch-multi-wave5-products') as typeof import('./data/shop-batch-multi-wave5-products');
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { seedShopDigikalaBatch1Part2Products } =
          require('./data/shop-digikala-batch1-part2-products') as typeof import('./data/shop-digikala-batch1-part2-products');
        seedShopDigikalaBatch1Part2Products();
      } catch (err) {
        console.warn('Shop Digikala batch1 Part2 seed skipped/failed:', (err as Error).message);
      }
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { seedShopDigikalaBatch1Part3Products } =
          require('./data/shop-digikala-batch1-part3-products') as typeof import('./data/shop-digikala-batch1-part3-products');
        seedShopDigikalaBatch1Part3Products();
      } catch (err) {
        console.warn('Shop Digikala batch1 Part3 seed skipped/failed:', (err as Error).message);
      }
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { seedShopDigikalaBatch1Part4Products } =
          require('./data/shop-digikala-batch1-part4-products') as typeof import('./data/shop-digikala-batch1-part4-products');
        seedShopDigikalaBatch1Part4Products();
      } catch (err) {
        console.warn('Shop Digikala batch1 Part4 seed skipped/failed:', (err as Error).message);
      }
    };

    const bootShopCatalogGuard = () => {
      // Delete leftover demo/seed SKUs. Never touches p221–p319 prices/images/stock.
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
...
function seedFinanceDefaults() {
  ...
  if (productCount === 0) {
    // Live SKUs (p221–p319) are seeded by bootShopPilot — never insert demo toys/beds/collars here.
    const cats = [
  ...
  switch (kind) {
    ...
    case 'support':
      return {
        ...
        systemFee: 0,
        systemReason: '',
        debitReason: kind === 'finance' ? 'مشاوره مالی' : 'پشتیبانی',
        payoutReason: kind === 'finance' ? 'مشاوره مالی' : 'پشتیبانی',
        payoutRefType: kind === 'finance' ? 'finance_consult' : 'support_consult',
      };
    default:
      return {
        cost: QUICK_VET_COST,
...
function mapVetConsultation(row: Record<string, unknown>): VetConsultation {
  ...
  return {
    ...
    patientBio: (() => {
      const bio = String(row.patient_bio ?? '').trim();
...
export const dbService = {
  ...
  payVetForAcceptedConsult(consultId: number): {
    ...
    // ISO with ms — SQLite datetime('now') is second-precision and can make the
    // 1s seeker-advice early-refund window look "too late" on the next tick (CI).
    ...
    const marked = db
      ...
      .run(paidAtIso, totalFee, providerAmount, consultId);
    if (marked.changes === 0) {
      return {
        paid: false,

...
export function getDb(): AppDatabase {
  if (!db) {
    ...
    const bootShopPilot = () => {
      // Additive Royal Canin pilot + Batch 2 + Batch 3 + Batch-multi wave 1 SKUs — upsert by slug; never wipe catalog.
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
    ...
    const bootShopCatalogGuard = () => {
      // Delete leftover demo/seed SKUs. Never touches p221–p259 prices/images/stock.
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
...
function seedFinanceDefaults() {
  ...
  if (productCount === 0) {
    // Live SKUs (p221–p259) are seeded by bootShopPilot — never insert demo toys/beds/collars here.
    const cats = [
